...
//Memory allocation on the basis of Optimal Future Checking
int PRAlgo_OPT(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
  ...
  for(int i=0;i<num_frames;i++)
  {
    //Store current_access
      temp = current_access;
    //We are going to store the current PageFrames id
      id = PageFrames[i].page_id;
		  access = PageAccesses[temp];

      while(id != access && temp < num_accesses)
      {
        temp++;
        access = PageAccesses[temp];
      }

      if (temp > futureAccess) 
      {
        futureAccess = temp;
        index = i;
		}
...
//Memory allocation on the basis of LRU
int PRAlgo_LRU(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
  int oldAccess  = PageFrames[0].time_of_access;
  int index=0;

  for(int i=0;i<num_frames;i++)
  {
    if (PageFrames[i].time_of_access < oldAccess) 
    {
			oldAccess = PageFrames[i].time_of_arrival;
			index = i;
		}
  }
...
//Memory allocation on the basis of NRU and Locality 
int PRAlgo_CUST(const PageFrame * PageFrames, int num_frames, const int * PageAccesses, int num_accesses, int current_access)
{
  int counter = 0;
  int recentPages[5] = {};
  /* Find the pages used in the past 5 cycles */
  for (int i = current_access; (i >= 0 && i > (current_access - 5)); i--)
  {
    recentPages[counter] = PageAccesses[i];
    counter++;
  }
  for (int j = 0; j< num_frames; j++)
  {
    ...
    for (int k = 0; k < 5; k++)
    {
      if (recentPages[k] == PageFrames[j].page_id)
      {
        flag = 1;
        break;
      }
